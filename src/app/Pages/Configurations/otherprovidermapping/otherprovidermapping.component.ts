...
@Component({
  selector: "app-otherprovidermapping",
  templateUrl: "./otherprovidermapping.component.html",
  styleUrls: ["./otherprovidermapping.component.css"],
})
export class OtherprovidermappingComponent implements OnInit, OnDestroy {
  ...
  public updateidtype: string = "";
  public updateidvalue: string = "";
  ...
  @ViewChild("AddOtherProvidermappingChild")
  private AddOtherProvidermappingChild: AddotherprovidermappingComponent;
          ...
          (data) => {
            if (data != null || data != undefined) {
              ...
              if (sdata != null && sdata != undefined) {
                ...
                this.Providermappingitems = <any>this.ProvidermappinggridData;
                ...
                this.api.insertActivityLog(
                  "Provider Other Mapping List Viewed",
  ...
  private loadProvidermappingitems(): void {
    try {
      if (this.Providermappingitems.length > 0) {
        this.ProvidermappinggridView = process(
          this.Providermappingitems,
          this.state
        );
      } else {
      ...
      //     total: this.Providermappingitems.length
      //   };
      // } else {
  ...
  EditProvidermapping({ sender, rowIndex, dataItem }) {
    try {
      this.editProvidermappingid = dataItem.additionalmappingid;
      this.InputEditMessage = "Do you want to edit provider other mapping?";
    } catch (error) {
      this.clsUtility.LogError(error);
    }
  }
  ...
  OutputEditResult($event) {
    try {
      ...
      if (this.OutEditResult == true) {
        ...
        $("#addotherprovidermappingModal").modal("show");
      }
    } catch (error) {
      this.clsUtility.LogError(error);
    }
  ...
  OutputProvidermappingEditResult($event) {
    try {
      ...
      if (IsSaved == true) {
        this.getProvidermappingById();
        this.AddOtherProvidermappingChild.ResetComponents();
  ...
  OnProvidermappingStatus(
    additionalProvidermappingid,
    additionalProvidermappingStatus
  ) {
    try {
      let objProvidermapping: Providermapping;
      objProvidermapping = new Providermapping();
      objProvidermapping.mappingid = additionalProvidermappingid;
      objProvidermapping.status = additionalProvidermappingStatus;
  ...
  updateStatusConfirmation(dataItem: any) {
    try {
      this.updateStatusMappingId = dataItem.additionalmappingid;
      this.deleteNPI = dataItem.npi;
      this.updateidtype = dataItem.idtype;
      this.updateidvalue = dataItem.idvalue;
      let status: boolean = dataItem.status;

      if (status) {
        this.updateMappingMessage =
          "Do you want to deactivate provider other mapping for " +
          dataItem.idtype +
          " and " +
          dataItem.idvalue +
          "?";
        ...
        this.updateMappingMessage =
          "Do you want to activate provider other mapping for " +
          dataItem.idtype +
          " and " +
          dataItem.idvalue +
          "?";
  ...
  updateStatusMapping() {
    try {
      let obj: { additionalmappingid: string; status: boolean } = {
        additionalmappingid: this.updateStatusMappingId,
        status: this.updateMappingStatus,
      };
      let seq1 = this.api.post(
        "SubClient/AdditionalProviderMapping/UpdateStatus",
        obj
      );
      this.subscription.add(
        seq1.subscribe(
            ...
            if (data != null || data != undefined) {
              if (data == 1) {
                this.clsUtility.showSuccess(
                  "Provider other mapping status updated successfully"
                );
                this.api.insertActivityLog(
                  "Provider other mapping for (NPI : " +
                    this.deleteNPI +
                    ", IdType : " +
                    this.updateidtype +
                    ", IdValue : " +
  ...
  public dataStateChange(state: DataStateChangeEvent): void {
    this.state = state;
    if (
      !isNullOrUndefined(this.Providermappingitems) &&
      this.Providermappingitems.length > 0
    ) {
