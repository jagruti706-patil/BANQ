...
@Component({
  selector: "app-unprocessmasterfiles",
  templateUrl: "./unprocessmasterfiles.component.html",
  styleUrls: ["./unprocessmasterfiles.component.css"],
})
export class UnprocessmasterfilesComponent implements OnInit, OnDestroy {
  constructor(
  ...
  public selectedMasterStatusValue: string = "2";
  public sMasterStatus: any = [{ value: "2", text: "Unprocess" }];
  public sMasterFileBreadCrumb: string = "MasterFiles";
  public items: any[] = [{ text: "Delete" }];
  public sSelectedMasterStatus: string = "2";
  ...
  deletemasterfileid: any; 
  deletemasterfilename: string = "";
  ...
  ngOnInit() {
    try {
      ...
      this.api.insertActivityLog(
        "Unprocessed Master Files List Viewed",
        "Files",
        "READ"
      );
    } catch (error) {
            ...
            (data) => {
              if (data != null && data != undefined) {
                ...
                if (status == "View") {
                  ...
                  this.api.insertActivityLog(
                    "Unprocessed Master File (" +
                      this.stitleMasterfile +
                      ") Viewed",
                    "Files",
                    "READ",
      ...
      this.subscription.add(
        this.filedetailService.getSplit835File(nerafileid).subscribe(
          (data) => {
            ...
            if (this.sSplitfile != null && this.sSplitfile != undefined) {
      ...
      this.subscription.add(
        seq.subscribe(
          (res) => {
            ...
            if (this.sAllClients != null && this.sAllClients.length > 0) {
      ...
      this.subscription.add(
        seq.subscribe(
          (res) => {
            ...
            if (this.sSubClients != null && this.sSubClients != undefined) {
  ...
  OnCellClickMasterFile(event: any) {
    try {
      ...
      //   if (event.dataItem.claimcount != "0") {
      //     if (
      //       this.searchfiltersvr.payerfilter == true &&
      //       this.searchfiltersvr.files_mastererafileid !=
      //         event.dataItem.nmastererafileid
      //     ) {
      //       this.searchfiltersvr.clearpayerilter();
  ...
  onMenuItemSelect(dataItem: any) {
    try {
      ...
      if (dataItem.nmastererafileid != "" && dataItem.nmastererafileid != null) {
        this.deletemasterfileid = dataItem.nmastererafileid;
        this.deletemasterfilename = dataItem.sfilename;
  ...
  onCloseDeleteConfirmationClick() {
    try {
  ...
  onYesDeleteConfirmationClick() {
    try{
      this.deletemasterfile();
    } catch (error) {
      this.clsUtility.LogError(error);
    }  
  }
  ...
  deletemasterfile(){
    try {
      this.subscription.add(
        this.filedetailService
          .DeleteMasterFile(
            this.deletemasterfileid
          )
          .subscribe(
            (data) => {
              if(data != null && data != undefined){
                if(data == "1"){
                  this.clsUtility.showSuccess("Master file deleted successfully");
                  $("#deleteconfirmationModal").modal("hide");
                  this.api.insertActivityLog(
                    this.deletemasterfilename + " master file deleted successfully",
                    "Files",
                    "READ"
                  );
                  this.RetriveMasterFiles_v2();
                } else {
                  this.clsUtility.showSuccess("Master file not deleted");
                }
              }
            },
            (err) => {
  ...
  clearFilters() {
    try {
      ...
      //   this.sSearchText
      // );
      this.searchfiltersvr.clearmasterfileunprocessfilter();
